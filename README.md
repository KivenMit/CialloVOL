...
**问题：MiniDump 文件扫描结果为空**
- 原因：该进程内存中没有可识别的文件路径/网络地址字符串
- 解决：使用「转储分析」标签页查看模块、线程、原始字符串，或使用内容查看器手动检查

**问题：Volatility 3 未找到**
- 原因：未安装或未配置路径
- 解决：前往「设置」标签页手动指定 `vol.exe` 或 `vol.py` 路径

**问题：完整内存镜像分析失败**
- 原因：文件损坏或格式不支持
- 解决：确认文件是完整物理内存镜像（非进程转储），尝试使用 Volatility 命令行验证

**问题：扫描时弹出 "Updating caches" 错误**
- 原因：旧版本将 Volatility 正常日志误判为错误
- 解决：已在 v1.2 修复，现在只报告真实错误

**问题：单进程 DMP 进程列表为空**
- 原因：旧版本未提取 MiscInfo 流中的 PID
- 解决：已在 v1.2 修复，现在自动提取真实 PID

**问题：网络连接页面点击 PID 无反应**
- 原因：旧版本未实现跳转功能
- 解决：已在 v1.2 新增，点击 PID 自动跳转到进程分析页面
